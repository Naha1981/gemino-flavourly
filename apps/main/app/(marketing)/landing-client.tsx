    ...
    <div className="min-h-screen bg-app-bg text-app-fg antialiased">
      ...
      <header className="border-b border-app-border/80">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6">
          ...
          <nav className="flex items-center gap-3">
            <Link href="/pricing" className="hidden sm:inline rounded-md px-3 py-2 text-sm font-medium text-app-muted hover:text-white">
              Pricing
            </Link>
            <SignedOut>
              <SignInButton forceRedirectUrl="/dashboard">
                <button className="rounded-md px-4 py-2 text-sm font-medium text-app-muted transition-colors hover:text-white">
                  Sign In
                </button>
              </SignInButton>
      ...
      <main>
        ...
        <section className="mx-auto max-w-4xl px-6 pb-16 pt-24 text-center">
          <p className="mb-6 inline-block rounded-full border border-app-border-strong bg-app-surface-0/60 px-4 py-1.5 text-xs font-medium text-app-muted shadow-sm">
            MADE FOR SOUTH AFRICAN RESTAURANTS
          </p>
          <h1 className="text-5xl font-bold leading-tight tracking-tight sm:text-6xl text-app-fg">
            Your restaurant, fully booked. While you cook.
          </h1>
          <p className="mx-auto mt-6 max-w-2xl text-lg leading-relaxed text-app-muted">
        ...
        <section className="mx-auto max-w-6xl px-6 pb-20">
          <h2 className="mb-10 text-center text-2xl font-semibold text-app-fg">Six engines, one AI assistant</h2>
        ...
        {/* ── How it works ──────────────────────────────── */}
        <section className="border-t border-app-border/80 py-20 bg-app-bg/50">
          <div className="mx-auto max-w-6xl px-6">
            <h2 className="text-center text-2xl font-semibold text-app-fg">Live in 5 minutes</h2>
        ...
        <section className="mx-auto max-w-6xl px-6 py-20">
          <div className="grid gap-6 md:grid-cols-3">
            <div className="flex items-start gap-3 rounded-lg border border-app-border bg-app-surface-0/50 p-5">
              <QrCode className="mt-0.5 h-5 w-5 shrink-0 text-emerald-400" />
              <div>
                <p className="font-semibold text-app-fg">No new app for guests</p>
                <p className="mt-1 text-sm text-app-muted">They message you on WhatsApp, exactly as they already do.</p>
              </div>
            </div>
            <div className="flex items-start gap-3 rounded-lg border border-app-border bg-app-surface-0/50 p-5">
              <ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-emerald-400" />
              <div>
                <p className="font-semibold text-app-fg">POPIA compliant</p>
                <p className="mt-1 text-sm text-app-muted">One-tap STOP opt-out, encrypted data, clear retention. Built for South Africa.</p>
              </div>
            </div>
            <div className="flex items-start gap-3 rounded-lg border border-app-border bg-app-surface-0/50 p-5">
              <TrendingUp className="mt-0.5 h-5 w-5 shrink-0 text-emerald-400" />
              <div>
                <p className="font-semibold text-app-fg">Pause anytime</p>
                <p className="mt-1 text-sm text-app-muted">A master kill-switch stops all AI messaging instantly. You stay in control.</p>
              </div>
            </div>
          </div>
        </section>
      ...
      <footer className="border-t border-app-border/80 py-8">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-6 text-sm text-app-faint">
          ...
          <div className="flex gap-5">
            <Link href="/pricing" className="transition-colors hover:text-app-muted">Pricing</Link>
            <Link href="/privacy" className="transition-colors hover:text-app-muted">Privacy</Link>
            <Link href="/terms" className="transition-colors hover:text-app-muted">Terms</Link>
            <Link href="/dashboard" className="transition-colors hover:text-app-muted">Tenant Portal</Link>
          </div>
        </div>
      </footer>
    </div>
...
function OutcomeCard({ icon: Icon, title, body }: { icon: LucideIcon; title: string; body: string }) {
  return (
    <div className="rounded-lg border border-app-border bg-app-surface-0/50 p-6 hover:border-app-border-strong transition-colors">
      <div className="mb-4 inline-flex rounded-md bg-app-surface-1 p-2.5">
        <Icon className="h-5 w-5 text-emerald-400" />
      </div>
      <h3 className="mb-2 font-semibold text-app-fg">{title}</h3>
      <p className="text-sm leading-relaxed text-app-muted">{body}</p>
    </div>
  );
}
...
function Step({ n, title, body }: { n: string; title: string; body: string }) {
  return (
    <div className="text-center space-y-2">
      <div className="mx-auto mb-4 flex h-10 w-10 items-center justify-center rounded-full border border-app-border-strong text-sm font-semibold text-emerald-400 bg-app-surface-0 shadow-inner">
        {n}
      </div>
      <h3 className="font-semibold text-app-fg text-base">{title}</h3>
      <p className="text-sm text-app-muted max-w-xs mx-auto leading-relaxed">{body}</p>
    </div>
  );
}
