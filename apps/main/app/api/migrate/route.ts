...
export async function GET() {
  ...
  try {
    ...
    `;
    await sql`CREATE INDEX IF NOT EXISTS marketing_events_tenant_idx ON marketing_events (tenant_id);`;
    ...
    await sql`ALTER TABLE tenants ADD COLUMN IF NOT EXISTS owner_user_id text;`;
    await sql`
    ...
    `;
    await sql`CREATE UNIQUE INDEX IF NOT EXISTS memberships_user_tenant_uniq ON memberships (user_id, tenant_id);`;
