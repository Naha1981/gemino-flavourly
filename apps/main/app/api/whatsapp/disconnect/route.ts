...
export async function POST() {
  ...
  try {
    await operatorClient.disconnect(tenant.id, account.id);
    return NextResponse.json({ ok: true, status: 'disconnected' }, { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Could not disconnect WhatsApp.' }, { status: 502 });
  }
}
