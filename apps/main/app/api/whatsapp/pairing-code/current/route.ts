...
export async function GET() {
  ...
  try {
    const pairing = await operatorClient.getPairingCode(tenant.id, account.id);
    return NextResponse.json(pairing, { headers: { 'Cache-Control': 'no-store' } });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : 'Could not read the pairing code.' }, { status: 502 });
  }
}
