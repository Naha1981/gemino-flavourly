      ...
      {alerts.map((alert) => {
        ...
        return (
          ...
          >
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <Link
                    ...
                    className="font-semibold text-app-fg hover:text-amber-300"
                  >
                ...
                <p className="mt-0.5 font-mono text-[11px] text-app-faint">{alert.customerPhone}</p>
            ...
            >
              <StickyNote className="h-3.5 w-3.5 text-app-faint shrink-0" />
              <input
                type="text"
                value={notes[alert.id] ?? ''}
                onChange={(e) => setNotes((prev) => ({ ...prev, [alert.id]: e.target.value }))}
                placeholder="Add a note..."
                className="flex-1 rounded-md border border-app-border bg-app-bg px-3 py-1.5 text-xs text-app-fg placeholder:text-app-faint focus:border-amber-600 focus:outline-none"
              />
              <button
                type="submit"
                disabled={busyId === alert.id || !(notes[alert.id] ?? '').trim()}
                className="rounded-md border border-app-border-strong bg-app-surface-1 px-3 py-1.5 text-xs font-medium text-app-fg hover:bg-app-surface-2 disabled:opacity-40"
              >
                Add note
              </button>
            </form>
