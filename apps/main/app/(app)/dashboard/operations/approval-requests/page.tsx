  ...
  return (
    <div className="space-y-6">
      <div className="border-b border-app-border pb-4">
        <h1 className="text-xl font-semibold text-app-fg">Approval Requests</h1>
        ...
        <div className="space-y-6">
          {pending.length > 0 && (
            <div className="space-y-3">
              ...
              {pending.map((req) => (
                <div key={req.id} className="rounded-lg border border-amber-900/50 bg-app-surface-0/70 p-4">
                  ...
                  <p className="mt-3 rounded-md border border-app-border bg-app-bg/50 p-3 text-sm text-app-fg">
                    {req.messageText}
                  </p>
                  <p className="mt-2 text-xs text-app-faint">Conversation: {req.conversationId}</p>
          ...
          {resolved.length > 0 && (
            <div className="space-y-3">
              <h2 className="text-sm font-medium text-app-muted">Resolved</h2>
              {resolved.map((req) => (
                <div key={req.id} className="rounded-lg border border-app-border bg-app-surface-0/50 p-4 opacity-75">
                  <div className="flex flex-wrap items-center gap-2 text-xs text-app-muted">
                    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs ${
                  ...
                  <p className="mt-3 rounded-md border border-app-border bg-app-bg/50 p-3 text-sm text-app-muted line-through decoration-zinc-600">
                    {req.messageText}
                  </p>
                  {req.approvedBy && (
                    <p className="mt-2 text-xs text-app-faint">By {req.approvedBy} on {new Date(req.approvedAt!).toLocaleString()}</p>
                  )}
                </div>
              ))}
            </div>
