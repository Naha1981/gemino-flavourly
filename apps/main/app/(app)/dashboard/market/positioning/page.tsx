...
const BAND_META: Record<string, { label: string; classes: string }> = {
  ...
  unknown: { label: 'Unknown', classes: 'border-app-border-strong bg-app-surface-1 text-app-muted' },
};
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-app-border bg-app-surface-0/50 p-4">
        <h2 className="mb-1 flex items-center gap-2 text-sm font-medium text-app-fg">
        ...
        <p className="mb-4 text-xs text-app-muted">{report.price.summary}</p>

        <ul className="space-y-2">
          {report.price.standings.map((entry) => (
            <li key={`${entry.name}-${entry.distanceKm ?? 'you'}`} className="flex items-center gap-3 text-xs">
              <span className={`w-40 shrink-0 truncate ${entry.isTenant ? 'font-semibold text-emerald-300' : 'text-app-muted'}`}>
                {entry.name}
                {entry.distanceKm !== null && (
                  <span className="ml-1 text-[10px] text-app-faint">{entry.distanceKm.toFixed(1)}km</span>
                )}
              </span>
              <span className="h-3 flex-1 overflow-hidden rounded bg-app-surface-1">
                <span
                  className={`block h-full ${entry.isTenant ? 'bg-emerald-500' : 'bg-app-surface-3'}`}
                  style={{ width: entry.average === null ? '0%' : `${Math.round((entry.average / maxAverage) * 100)}%` }}
                />
              </span>
              <span className="w-16 shrink-0 text-right text-app-muted">
      ...
      <section className="rounded-lg border border-app-border bg-app-surface-0/50 p-4">
        ...
        <p className="mb-4 text-xs text-app-muted">{report.rating.summary}</p>
          ...
          {report.rating.standings.map((entry, index) => (
            <li
              ...
              className={`flex items-center gap-3 rounded-md px-2 py-1.5 text-xs ${
                entry.isTenant ? 'bg-emerald-950/40 text-emerald-200' : 'text-app-muted'
              }`}
            >
              <span className="w-6 shrink-0 text-right text-app-faint">{entry.rating === null ? '—' : index + 1}</span>
              ...
              <span className="w-20 shrink-0 text-right text-[10px] text-app-faint">
      ...
      <section className="rounded-lg border border-app-border bg-app-surface-0/50 p-4">
        ...
        <p className="mb-4 text-xs text-app-muted">{report.menu_overlap.summary}</p>

        {report.menu_overlap.per_competitor.length === 0 ? (
          <p className="text-xs text-app-faint">No competitors tracked yet.</p>
        ) : (
          <ul className="space-y-3">
            {report.menu_overlap.per_competitor.map((entry) => (
              <li key={entry.competitorId}>
                <div className="flex items-center gap-3 text-xs">
                  <span className="w-40 shrink-0 truncate text-app-muted">{entry.competitorName}</span>
                  <span className="h-2 flex-1 overflow-hidden rounded bg-app-surface-1">
                    <span
                      className="block h-full bg-emerald-600"
                      style={{ width: entry.overlapPercent === null ? '0%' : `${entry.overlapPercent}%` }}
                    />
                  </span>
                  <span className="w-24 shrink-0 text-right text-app-muted">
                ...
                {entry.sharedItems.length > 0 && (
                  <p className="mt-1 pl-40 text-[10px] text-app-faint">shared: {entry.sharedItems.join(', ')}</p>
                )}
              </li>
            ))}
          </ul>
      ...
      <section className="rounded-lg border border-app-border bg-app-surface-0/50 p-4">
        ...
        <p className="mb-3 text-xs text-app-muted">{report.unique_offerings.summary}</p>
